...
async function dockerCollect(args, opts = {}) { return execCollect("docker", args, opts); }
...
async function pullImage(ref) { try { await dockerCollect(["pull", ref]); } catch (e) { log("[docker] pull failed:", e && e.message); } }
...
async function httpGetJSON(url) { const buf = await httpGetBuffer(url); return JSON.parse(buf.toString("utf8")); }
...
function writeMinecraftScaffold(serverDir, name, fork, version) {
  try { fs.writeFileSync(path.join(serverDir, "eula.txt"), "eula=true\n", "utf8"); } catch {}
  // setăm explicit portul intern 25565 și server-ip gol
  const props = [
    `motd=${name}`,
    `max-players=20`,
  ...
  try { fs.writeFileSync(path.join(serverDir, "server.properties"), props, "utf8"); } catch {}
  const meta = { type: "minecraft", fork, version, start: "server.jar", createdAt: Date.now() };
  try { fs.writeFileSync(path.join(serverDir, "adpanel.json"), JSON.stringify(meta, null, 2), "utf8"); } catch {}
}
...
function fixedJarUrlFor1218(fork) {
  ...
  if (f === "paper") return "https://fill-data.papermc.io/v1/objects/8de7c52c3b02403503d16fac58003f1efef7dd7a0256786843927fa92ee57f1e/paper-1.21.8-60.jar";
  if (f === "pufferfish") return "https://ci.pufferfish.host/job/Pufferfish-1.21/33/artifact/pufferfish-server/build/libs/pufferfish-paperclip-1.21.8-R0.1-SNAPSHOT-mojmap.jar";
  if (f === "vanilla") return "https://piston-data.mojang.com/v1/objects/95495a7f485eedd84ce928cef5e223b757d2f764/server.jar";
...
// !!! route publică; dacă vrei HMAC, adaugă checkSignature la început
app.post('/v1/servers/:name/apply-version', async (req, res) => {
  try {
    ...
    try { await dockerCollect(['stop', name]); } catch {}
    await downloadToFile(url, path.join(baseDir, "server.jar"));
    ...
    // forțează server.properties correct
    ...
    const args = [
      ...
      // NOTĂ: nu mai setăm SERVER_PORT – portul intern rămâne 25565
      'itzg/minecraft-server:latest'
    ];
    await execCollect('docker', args);

    return res.json({ ok:true, msg:'applied', server: name, hostPort: Number(hostPort) });
  } catch (e) {
...
app.delete("/v1/servers/:name", async (req, res) => {
  ...
  try {
    try { await dockerCollect(["rm", "-f", name]); } catch {}
    try { fs.rmSync(path.join(VOLUMES_DIR, name), { recursive: true, force: true }); } catch {}
    ...
    if (templateId === "minecraft") {
      ...
      if (autoStart) {
        try {
          ...
          return res.status(500).json({ error: "created but failed to start container: " + (e && e.message) });
        }
      }
    } else {
...
app.post("/v1/servers/:name/stop", async (req, res) => {
  ...
  try {
    ...
    // Try graceful for Minecraft
    try { await dockerCollect(["exec", "--user", "1000", name, "mc-send-to-console", "stop"]); } catch {}
    setTimeout(async () => { try { await dockerCollect(["stop", name]); } catch {} }, 20000);
...
app.post("/v1/servers/:name/command", async (req, res) => {
  ...
  try {
    const serverDir = path.join(VOLUMES_DIR, name);
    if (!fs.existsSync(serverDir)) return res.status(404).json({ error: "not found" });
    if (!(await containerExists(name))) return res.status(400).json({ error: "container not running" });

    try {
      await dockerCollect(["exec", "--user", "1000", name, "/usr/local/bin/mc-send-to-console", command]);
      return res.json({ ok: true, method: "pipe" });
    } catch (e1) {
      try {
        await dockerCollect(["exec", "--user", "1000", name, "rcon-cli", command]);
        return res.json({ ok: true, method: "rcon" });
      } catch (e2) {
        return res.status(500).json({ error: "failed-to-send", detail: (e2 && e2.message) || (e1 && e1.message) || "unknown" });
      }
    }
  } catch (e) {
    res.status(500).json({ error: e.message || "failed" });
...
app.get("/v1/servers/:name/logs", (req, res) => {
  ...
  const send = (payload) => res.write(`data: ${JSON.stringify({ line: cleanLog(name, payload) })}\n\n`);
  ...
  if (!entry) {
    ...
    const spawnFollow = () => {
      ...
      try { if (entry.proc) { try { entry.proc.kill("SIGTERM"); } catch {} entry.proc = null; } } catch {}
    ...
    const scheduleWaitAndRetry = () => {
      ...
      entry.retryTimer = setInterval(() => {
        if (entry.closed) { try { clearInterval(entry.retryTimer); } catch {} entry.retryTimer = null; return; }
        dockerCollect(["inspect", "-f", "{{.State.Running}}", name])
          .then(({ out }) => {
            const running = (out || "").trim() === "true";
            if (running) {
              try { clearInterval(entry.retryTimer); } catch {}
    ...
    containerExists(name)
      .then((exists) => { exists ? spawnFollow() : scheduleWaitAndRetry(); })
  ...
  req.on("close", () => {
    ...
    if (entry.clients.size === 0) {
      entry.closed = true;
      if (entry.retryTimer) { try { clearInterval(entry.retryTimer); } catch {} entry.retryTimer = null; }
      if (entry.proc) { try { entry.proc.kill("SIGTERM"); } catch {} entry.proc = null; }
...
async function startMinecraftContainer(name, serverDir, hostPort) {
  ...
  return new Promise((resolve, reject) => {
    ...
    p.stderr.on("data", d => { err += d.toString(); log("[docker run err]", d.toString().trim()); });
    p.on("error", reject);
    p.on("close", (code) => {
...
app.post("/v1/fs/list", async (req, res) => {
  try {
    ...
    if (!fs.existsSync(abs) || !fs.statSync(abs).isDirectory()) return res.status(400).json({ ok: false, error: "not a directory" });
...
app.post("/v1/fs/write", async (req, res) => {
  try {
    ...
    const buf = encoding === "utf8" ? Buffer.from(String(content || ""), "utf8") : Buffer.from(String(content || ""), "base64");
...
app.post("/v1/fs/delete", async (req, res) => {
  try {
    ...
    if (isDir) fs.rmSync(abs, { recursive: true, force: true }); else fs.rmSync(abs, { force: true });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message || "failed" });
  }
...
app.post("/v1/fs/extract", async (req, res) => {
  try {
    ...
    let args, cmd = null;
    if (type === "zip") { cmd = "unzip"; args = ["-oq", abs, "-d", dir]; }
    else if (type === "tar") { cmd = "tar"; args = ["-xf", abs, "-C", dir]; }
    else if (type === "targz") { cmd = "tar"; args = ["-xzf", abs, "-C", dir]; }
    else if (type === "tarbz2") { cmd = "tar"; args = ["-xjf", abs, "-C", dir]; }
    else if (type === "7z") { cmd = "7z"; args = ["x", "-y", abs, `-o${dir}`]; }
    else if (type === "rar") { cmd = "unrar"; args = ["x", "-o+", abs, dir]; }
...
async function startMinecraftContainerWithOverrides(name, serverDir, hostPort, overrides = {}) {
  ...
  const portList = Array.isArray(overrides.ports) && overrides.ports.length ? overrides.ports : [`${Number(hostPort)}:25565`];
  ...
  return new Promise((resolve, reject) => {
    ...
    p.stderr.on("data", d => { err += d.toString(); log("[docker run err]", d.toString().trim()); });
    p.on("error", reject);
    p.on("close", (code) => (code === 0 ? resolve(true) : reject(new Error(err || "failed to start container"))));
  });
}
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({ ok: false, error: "failed-to-send", detail: (e2 && e2.message) || (e1 && e1.message) || "unknown" });
      }
    }
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message || "failed" });
...
app.get("/v1/servers/:name/files/read", async (req, res) => {
  try {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) return res.status(404).json({ error: "file not found" });
...
function shutdown() {
  ...
  try { httpServer.close(() => console.log("[node] http closed")); } catch {}
  try { sftpServer.close(() => console.log("[node] sftp closed")); } catch {}
  process.exit(0);
}
