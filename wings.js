...
app.delete("/v1/servers/:name", async (req, res) => {
  ...
  try {
    // 1) Oprește tail-ul de loguri, dacă există
    const entry = logTails.get(name);
    if (entry) {
      entry.closed = true;
      if (entry.retryTimer) {
    ...
    // 2) Dezactivează restart și șterge containerul (dacă există)
    try {
      // similar cu /v1/servers/:name/kill
      await execCollect("docker", ["update", "--restart=no", name]).catch(() => {});
    } catch {}
    try {
      await ensureNoContainer(name); // face docker rm -f name, cu catch intern
    } catch {}

    // 3) Șterge directorul de pe disk (volumes/<name>)
    const serverDir = path.join(VOLUMES_DIR, name);
    const safeDir = safeUnderVolumes(serverDir);
    if (safeDir && fs.existsSync(safeDir)) {
      try {
        fs.rmSync(safeDir, { recursive: true, force: true });
      } catch (e) {
        // dacă nu reușește, considerăm totuși eroare reală
        return res.status(500).json({ ok: false, error: e.message || "failed to remove directory" });
      }
    }

    // 4) Răspuns compatibil cu panelul
...
app.post("/api/nodes/server/:name/action", async (req, res) => {
  try {
    ...
    if (cmd === "start") {
      if (ctx.node) seedRuntimeOnNode(ctx.node, req.params.name, hostPort).catch(() => {});
...
app.get("/v1/servers/:name/files/list", async (req, res) => {
  try {
    ...
    if (finalCmd === "start") {
      seedRuntimeOnNode(node, name, hostPort).catch(() => {});
      path = `/v1/servers/${encodeURIComponent(name)}/start`;
...
app.delete("/v1/servers/:name/files/delete", async (req, res) => {
  try {
    ...
    if (cmd === "start") {
      ...
      seedRuntimeOnNode(node, req.params.name, hostPort).catch(() => {});
      path = `/v1/servers/${encodeURIComponent(req.params.name)}/start`;
      payload = startPayloadFor(req.params.name, hostPort);
    } else if (cmd === "stop") {
      path = `/v1/servers/${encodeURIComponent(req.params.name)}/stop`;
