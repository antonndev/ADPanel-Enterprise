...
async function applyRuntimeChange(name, body, defaultTemplate) {
  const { runtime, template, start, port } = body || {};
  const paths = resolveServerPaths(name);
  if (!paths) return { status: 404, json: { ok: false, error: "not-found" } };

  if (!runtime || !runtime.image || !runtime.tag) {
    return { status: 400, json: { ok: false, error: "missing-runtime" } };
  }

  const templateId = template || runtime.template || defaultTemplate || "nodejs";
  ...
  return { status: 200, json: { ok: true, runtime: nextMeta.runtime, meta: nextMeta } };
}

app.post("/v1/servers/:name/runtime", authMiddleware, async (req, res) => {
  try {
    const result = await applyRuntimeChange(req.params.name, req.body, "nodejs");
...
app.post("/v1/servers/:name/node-version", authMiddleware, async (req, res) => {
  try {
    const body = Object.assign({}, req.body || {}, {
      template: req.body?.template || "nodejs",
      runtime: req.body?.runtime || { image: "node", tag: req.body?.version || "latest", command: req.body?.command }
    });
    const result = await applyRuntimeChange(req.params.name, body, "nodejs");
    return res.status(result.status).json(result.json);
  } catch (e) {
    console.error("[node] node-version apply failed", e);
    res.status(500).json({ ok: false, error: "runtime-change-failed" });
  }
});

app.post("/v1/servers/:name/python-version", authMiddleware, async (req, res) => {
  try {
    const body = Object.assign({}, req.body || {}, {
      template: "python",
      runtime: req.body?.runtime || { image: "python", tag: req.body?.version || "latest", command: req.body?.command }
    });
    const result = await applyRuntimeChange(req.params.name, body, "python");
    return res.status(result.status).json(result.json);
  } catch (e) {
    console.error("[node] python-version apply failed", e);
    res.status(500).json({ ok: false, error: "runtime-change-failed" });
  }
});
